...
# from transformer.transformers import TransformerModel
...
class Trainer(BaseTrainer):
    def __init__(self, cfg, args):
        ...
        self.rotation_net_fc = dn_lib.RotationNet_FC(cfg.models.rotationnet)
        self.rotation_net_fc.cuda()
        print("rotationnet_fc:")
        print(self.rotation_net_fc)

        dn_lib_y = importlib.import_module(cfg.models.rotationnet_y.type)
        self.rotation_net_y_fc = dn_lib_y.RotationNet_FC(cfg.models.rotationnet_y)
        self.rotation_net_y_fc.cuda()
        print("rotationnet_y_fc:")
        print(self.rotation_net_y_fc)

        dn_lib_z = importlib.import_module(cfg.models.rotationnet_z.type)
        self.rotation_net_z_fc = dn_lib_z.RotationNet_FC(cfg.models.rotationnet_z)
        self.rotation_net_z_fc.cuda()
        print("rotationnet_z_fc:")
        print(self.rotation_net_z_fc)
    ...
    def update(self, data_full, no_update=False):
        ...
        # pairwise operation (TO DO: ADD transformer)
        if self.pairwise_type == "concat":
            ...
            #2D
            # pairwise_feature = torch.cat([image_feature_map1, image_feature_map2], dim=1)
            # pairwise_feature = pairwise_feature.view((pairwise_feature.shape[0], pairwise_feature.shape[2], pairwise_feature.shape[3], pairwise_feature.shape[1]))
            ...
            trans_model = MultiHeadSelfAttention(dim=emsize)
            trans_output = trans_model(pairwise_feature)

            trans_model = TransformerModel(ntokens, emsize, nhead, d_hid, nlayers, dropout).to(device)
            trans_output = trans_model( pairwise_feature )
            out_cls = trans_output[0, :, :]   # out_cls is [batch_size, 512]
            # TO DO: ADD A FULLY CONNECTED NETWORK THAT TAKES THE trans_output and predict the angle
            trans_output = out_cls
            # trans_output = trans_output.view(trans_output.shape[1], trans_output.shape[0], trans_output.shape[2])

        # loss type
        if not self.classification:
            # regression loss
            out_rmat, out_rotation = self.rotation_net(pairwise_feature) # OLD
            # out_rmat, out_rotation = self.rotation_net_fc(out_cls)
            ...
            _, out_rotation_x = self.rotation_net_fc(trans_output)
            _, out_rotation_y = self.rotation_net_y_fc(trans_output)
            _, out_rotation_z = self.rotation_net_z_fc(trans_output)
        ...
        with torch.no_grad():
            ...
            for data_full in tqdm.tqdm(test_loader):
                ...
                if self.pairwise_type == "concat":
                    ...
                    pairwise_feature = torch.cat([image_feature_map1, image_feature_map2], dim=2)
                    pairwise_feature = pairwise_feature.view(
                        (pairwise_feature.shape[2], pairwise_feature.shape[0], pairwise_feature.shape[1]))
